...
/*  
  for(int i=0; i<width*height; i++){
    _x = ((i%width)-x0)* *(*ptr+rot) - ((i/width)-y0)* *(*(ptr+1)+rot) + _x0;
    _y = ((i%width)-x0)* *(*(ptr+1)+rot) + ((i/width)-y0)* *(*ptr+rot) + _y0;
    if(((_x>=0)&&(_x<=(*new_width)-1))&&((_y>=0)&&(_y<=(*new_height)-1))){
      *(new_Img+(_y* (*new_width) + (_x+1)-1)) = *(img+i);
    }
